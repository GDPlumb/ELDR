...
def train_multiclass(x, y, num_classes,
    ...
    with tf.variable_scope("learner_model", reuse = tf.AUTO_REUSE):
        ...
        grad_i = tf.gradients(prob_i, X)

    with tf.variable_scope("learner_model", reuse = tf.AUTO_REUSE):
        pred_from_rep = learner.model(R)

    # Define the loss and optimizer
    model_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels = Y, logits = pred))
    tf.summary.scalar("Cross Entropy", model_loss)
    
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(pred, axis = 1), Y), tf.float32))
